...
void trialMove(Pos*position,int N,double L,double *E_P,int *accept_cnt,double T){
	/*Attempt to perform Trial move of random particle o with random displacement*/
	int o;
	double dr,randNr,beta,enOld,enNew,ranX,ranY,ranZ;
	...
	part.x = (position->pos_x)[o]+ (ranX - 0.5)*dr;
	part.y = (position->pos_y)[o]+ (ranY - 0.5)*dr;
	part.z = (position->pos_z)[o]+ (ranZ - 0.5)*dr;
...
int main(){
	...
	while(!done){
		trialMove(&positions,N,L,&E_P,&accept_cnt,T);
		loadingBar(i,cycles);
		if(!(i%sample)){
	...
	printf("\n Accept ratio: %lf \n", ((double)accept_cnt)/cycles);
	printf("delta g: %lf L: %lf \n",delg,L);
